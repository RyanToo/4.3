...
function login($login, $root, $db){

	try {
    $q = $db->query('SELECT id, login, root FROM user');
    ...
    foreach($row as $regUser){
        if (($regUser['login'] == $login) && (md5($root) == $regUser['root'])) {
            unset($regUser['root']);
...
class Registration
{
	...
	public function __construct($login, $root, $db)
	{
		$this->login = $login;
		$this->root = $root;
		$this->db = $db;
	...
	public function createUser()
	{
		...
		$root = trim(htmlspecialchars($this->root));

		if((strlen($login) > 0) && (strlen($root) > 0)){
			$root = md5($root);
			try {
				$q = $db->prepare('INSERT INTO user (login,root) VALUES (?,?)');
				$q->execute(array($login, $root));
